---
name: start-feature
description: Start a new feature from a GitHub Issue — creates a git worktree or branch (your choice), installs dependencies, and kicks off TDD by scaffolding test files before any implementation.
...
Once confirmed, ask the user:

> **Worktree or branch?**
> - **Worktree** — creates an isolated sibling directory (`../FEATURE_NAME`). Best for running multiple agents or keeping main unaffected while you work.
> - **Branch** — checks out a new branch in the current repo directory. Simpler for solo work.

Record the choice as `MODE` (`worktree` or `branch`). All subsequent steps adapt based on this choice.

Then **immediately create `PLAN.md`** in the repo root as a scratchpad (it will be committed in Step 5). Populate it with everything you already know:
...
## Step 2 — Create the worktree or branch
...
## Step 3 — Install frontend dependencies *(worktree only — skip for branch)*
...
**If MODE = worktree** (run from sibling directory):
...
Tick off the completed checklist items in `PLAN.md` (worktree/branch created, deps installed), then stage and commit **only the test files and PLAN.md** — no production code yet.
...
**If MODE = branch**, print:

```
✔ Branch created:    dev/FEATURE_NAME  (in current repo)
✔ Tests scaffolded:  [list files created]
✔ PLAN.md written:   PLAN.md
✔ Test commit:       [commit hash]

Next steps (implement to green):
  cat PLAN.md   ← start here
  [implement each acceptance criterion until all tests pass]
  ./scripts/run-unit-tests.sh   ← run before pushing
...
- Never merge to main — always end with a PR.
- If the issue number does not exist or the `gh` command fails, stop and tell the user clearly.
- If the worktree directory or branch already exists, stop and ask the user before overwriting.
