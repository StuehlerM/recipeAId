...
| POST | `/api/v1/recipes` | Create recipe (JSON) — body includes optional `servings` (positive int); rejected with 400 if ≤ 0 |
| PUT | `/api/v1/recipes/{id}` | Update recipe — same `servings` validation applies; pass `null` to clear |
