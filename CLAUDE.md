...
**Ingredient parser architecture:** `LlmIngredientParserService` (in `RecipeAId.Api/ParserServices/`) implements `IIngredientParserService` by calling the Ministral 3B sidecar at `IngredientParser:BaseUrl` (default `http://localhost:8002`, 200-second timeout). **Async SSE pipeline (Step 2 only):** `POST /from-image` accepts a `?refine=true` query param (default true). When true, after OCR + regex parse, the endpoint creates an `OcrSession` in `OcrSessionStore` (singleton `ConcurrentDictionary`), fires the LLM as a background `Task.Run` using a fresh DI scope (`IServiceScopeFactory`), and returns immediately with the regex draft + `sessionId`. The frontend then opens `GET /api/v1/ocr-sessions/{sessionId}/events` (SSE) and waits for `{"status":"done","ingredients":[...]}` or `{"status":"failed"}`. On failure the frontend silently uses the regex draft. Steps 1 (Title) and 3 (Instructions) pass `?refine=false`, skipping LLM entirely and returning the regex draft immediately. `OcrSessionCleanupService` (BackgroundService) purges sessions older than 5 minutes every 60 s. `IIngredientParserService.ParseAsync` is also exposed standalone via `POST /api/v1/ingredients/parse`. **SSE health polling:** The `OcrSessionsController.StreamEvents` handler (SSE endpoint) polls the ingredient-parser's `/status` endpoint every 30 seconds to monitor Ollama reachability. The connection remains open as long as Ollama is reachable, with a hard timeout of 15 minutes (900s). If the parser becomes unreachable after being reachable, the SSE stream fails with `"error":"ingredient parser lost"`. This allows slow LLM requests (which can take 180+ seconds on CPU) to complete without premature timeout. The Python sidecar applies 4-layer prompt injection defense: (1) `sanitizer.py` strips control chars, truncates to 2000 chars, removes role markers and injection phrases; (2) `prompt.py` wraps user text in `<ingredients>` XML delimiters with a hardcoded system prompt; (3) Pydantic schema validation of LLM output (name/value/unit only); (4) semantic sanity bounds: value clamped 0–5000, unit cleared if not in allow-list, name truncated at 100 chars, max 50 items. **Retry logic:** `_call_ollama` retries up to 3 times with exponential backoff (1s, 2s, 4s) on transient `httpx.HTTPError` failures, handling cases where Ollama is briefly unavailable. Model weights (~4 GB) stored in `ollama-models` Docker volume. **Endpoints:** `POST /parse` (ingredient parsing), `GET /health` (Ollama reachable + model loaded, includes `active_requests` count), `GET /status` (detailed state: `ollama_reachable`, `active_requests`, `processing` boolean). **Logging:** `sanitizer.py` logs truncation events and detected role markers/injection phrases at WARNING level; `main.py` logs LLM input prompt and raw output at INFO level, with per-attempt retry warnings on transient failures.
