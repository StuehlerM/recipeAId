...
**Frontend API client (`src/api/client.ts`):** uses `VITE_API_BASE_URL` to toggle between real fetch calls and mock data. All endpoints — including OCR — fall back to mock data when `VITE_API_BASE_URL` is not set. `checkOk` logs `[API] <label> → <status>` to `console.error` on non-2xx responses. `useOcrCapture` logs `[OCR]` upload/result info and errors. `CameraCapture` logs `[Camera]` permission denial vs other failures.
