...
The user wants to start work on a GitHub Issue. Extract the issue number from their message.
...
git commit -m "test(#<ISSUE_NUMBER>): scaffold failing tests for <FEATURE_NAME>"
```
...
- Never create production code in this step — only test scaffolds.
